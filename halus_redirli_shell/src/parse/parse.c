...
/*   Updated: 2025/08/25 15:03:34 by makboga          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_display_info(t_shell *shell)
{
	...
	if (path)
	{
		properties = string_concatation((char *[]){"\033[1;34m",
								user, "@", hostname,":", "\033[1;35m", path, "\033[0m", "$ ", NULL});
		if (properties)
		{
...
int	get_prompt(t_shell *shell)
{
	...
	new_prompt = readline(shell->display_info);
	shell->prompt = set_and_free(shell->prompt, new_prompt);
	if(!shell->prompt)
	{
		printf("exit\n");
		return 0;
